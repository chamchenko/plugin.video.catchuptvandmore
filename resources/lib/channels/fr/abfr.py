...
URL_AUTH_CALLBACK = 'https://www.bistvonline.com/auth-callback'

URL_API = 'https://subscription.digital.api.abweb.com/api/subscription/has-live-rights/%s/bistv-online'
...
def get_live_url(plugin, item_id, **kwargs):
    ...
    params = {
        'client_id': 'bistv-online',
        'redirect_uri': URL_AUTH_CALLBACK,
        'response_type': 'code',
    ...
    resp4 = session_requests.get(URL_API % (item_id), headers=headers)
    json_parser4 = json.loads(resp4.text)
    if json_parser4['subscriptionRequestStatus'] == 0:
        xbmcgui.Dialog().ok(
            'Info',
            plugin.localize(30607))
        return False
    else:
